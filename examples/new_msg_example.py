...
)  # можем указать дополнительный фильтр, в таком случае будет выполняться только в том случае, если функция-предикат вернёт True
...
@runner.add_handler(
    MessageTypes.NEW_MESSAGE, my_filter
)  # указываем нашу функцию-предикат в качестве фильтра
def msg_hook_with_filter(msg: NewMessageEvent) -> None:
    ...
    print(
        f"Пользователь {msg.author.username} отправил сообщение, которое начинается с !"
    )
